...
// SAIPA Vehicle Models from user's exact list
const saipaModels = [
  // Pride Series
  ...
  { name: 'آریو (مونتاژ زوتی Z300)', nameEn: 'Aryo (Zotye Z300)', category: 'سدان', engine: '1500cc', year: '2018-2020' },
  { name: 'سیتروئن ژیان (مونتاژ)', nameEn: 'Citroen Xyan Assembly', category: 'سدان', engine: '1600cc', year: '2005-2010' },
  { name: 'سیتروئن زانتیا (مونتاژ)', nameEn: 'Citroen Xantia Assembly', category: 'سدان', engine: '1800cc', year: '1998-2005' },
  { name: 'رنو 5 (مونتاژ)', nameEn: 'Renault 5 Assembly', category: 'هاچبک', engine: '1400cc', year: '1985-1995' },
  { name: 'رنو 21 (مونتاژ)', nameEn: 'Renault 21 Assembly', category: 'سدان', engine: '1700cc', year: '1990-2000' },
  
  // KIA Assembly
  { name: 'کیا پراید (مونتاژ)', nameEn: 'KIA Pride Assembly', category: 'هاچبک', engine: '1300cc', year: '1993-2005' },
  { name: 'کیا ریو (مونتاژ)', nameEn: 'KIA Rio Assembly', category: 'هاچبک', engine: '1500cc', year: '2005-2010' },
  ...
  { name: 'کوییک R تیپ GX-L', nameEn: 'Quick R Type GX-L', category: 'هاچبک', engine: '1500cc', year: '2018-2020' },
...
const addSaipaModels = async () => {
  try {
    console.log('🚀 Starting SAIPA Models Import (Step 1)...\n');
    ...
    console.log(`📋 Processing ${saipaModels.length} SAIPA models...\n`);
    ...
    for (const [index, modelData] of saipaModels.entries()) {
      try {
        ...
        if (existingModel) {
          console.log(`${(index + 1).toString().padStart(3)}. ⏭️  ${modelData.name} (already exists)`);
          skipped++;
          continue;
        }
        ...
        console.log(`${(index + 1).toString().padStart(3)}. ✅ ${vehicleModel.name}`);
        added++;
        
      } catch (error) {
        console.log(`${(index + 1).toString().padStart(3)}. ❌ Failed to add ${modelData.name}: ${error.message}`);
        errors++;
      }
    }
    ...
    // Final Summary
    console.log('\n' + '═'.repeat(60));
    console.log('🎉 SAIPA MODELS IMPORT COMPLETED! 🎉');
    console.log('═'.repeat(60));
    console.log(`✅ Successfully added: ${added} models`);
    console.log(`⏭️  Already existed: ${skipped} models`);
    console.log(`❌ Errors: ${errors} models`);
    console.log(`📊 Total processed: ${added + skipped + errors} models`);
    
    // Verify database state
    const totalSaipaModels = await VehicleModel.countDocuments({ manufacturer: saipaManufacturer._id });
    console.log(`🔍 Total SAIPA models in database: ${totalSaipaModels}`);
    
    if (totalSaipaModels >= 70) {
      console.log('🎯 EXCELLENT! SAIPA database is now comprehensive.');
    } else if (totalSaipaModels >= 50) {
      console.log('👍 GOOD! SAIPA database has substantial coverage.');
    } else {
      console.log('⚠️  More SAIPA models may need to be added.');
    }
    
    console.log('\n✅ Ready to proceed to next manufacturer!');
