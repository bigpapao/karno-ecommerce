...
export const setDefaultAddress = asyncHandler(async (req, res, next) => {
  ...
  user.addresses.forEach((addr) => {
    addr.isDefaultAddress = addr._id.toString() === address._id.toString();
  });
...
export const validateAddressData = asyncHandler(async (req, res, next) => {
  ...
  // Use the validation service for more detailed results
  try {
    const validationResult = await addressValidationService.validateAddress({
      ...
      state,
      zipCode,
      country: country || 'IR' // Default to Iran
    });
    ...
    logger.error({
      ...
      error: error.message,
      stack: error.stack,
      requestData: { address, city, state, zipCode, country }
    });
    
    return next(new AppError(
      'Error validating address: ' + (error.message || 'Unknown error'),
      500,
